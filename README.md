...
- **LANG** (в `.env`) — язык по умолчанию (fallback). Также определяет, из какой папки читается список библиотек.
...
- **библиотеки** — все `.json` файлы в папке языка по умолчанию `i18n/{LANG}/`.
